...
Expected: `ModuleNotFoundError: No module named 'app'` or `ImportError`.
  ...
  <verify>
    <automated>cd services/watcher &amp;&amp; python -m pytest tests/test_extension_filter.py tests/test_minio_key_scheme.py -v 2>&1 | grep -E "ERROR|FAILED|ModuleNotFoundError" | head -10</automated>
  </verify>
  <done>All test collection fails with ImportError or ModuleNotFoundError (RED confirmed — app/ does not exist yet). At least one ERROR line visible in output.</done>
</task>
...
<task type="tdd">
  ...
  <behavior>
    ...
    - _make_minio_key(filename, mtime=None): mtime-based key when mtime provided (startup_scan), datetime.now()-based when None (live event)
    ...
    - startup_scan(watch_dir): iterates watch_dir, calls process_file(str(f), mtime=f.stat().st_mtime) for each video file — mtime makes key deterministic for dedup
